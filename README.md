...
fastest uncached in both suites, though only narrowly over props_template
(1.20x here, 1.06x on the 5MB document).
...
the other two. TurboStreamer's Oj encoder is still the fastest cached in both
suites, by 1.28x on the 22KB document and 2.76x on the 5MB one. The GC panels are normalized per iteration, not per
...
Rebuilt every iteration, RABL leads by roughly 3.5x (10,411 i/s against
TurboStreamer's 2,906): its template evaluates to a Ruby Hash that Oj serializes
...
three builder DSLs land close together — TurboStreamer 2,906, props_template
2,423, jbuilder 1,806.
...
the line of *what* each library caches. TurboStreamer (109,535 i/s) and
props_template (85,745) cache the fragment's serialized JSON and splice those
bytes into the output, so a hit costs a copy and no serialization. RABL (5,069)
and jbuilder (4,725) cache the data structure — RABL's key is
...
twenty-two-fold difference between the two groups, against 1.28x between the two
byte-cachers.
...
Rebuilt every iteration, RABL leads by about 2x (70.7 i/s against
TurboStreamer's 36.8): one Oj pass over a finished object graph beats roughly
51,000 Ruby-level DSL calls. The builder DSLs are again close, and closer than
at 22KB — props_template 34.7, jbuilder 22.5. Note the RSS panel, where building
...
re-emit 5MB of cached bytes — TurboStreamer 7,511 i/s, props_template 2,724 —
while RABL (53.2) and jbuilder (44.5) deserialize and re-serialize 5MB of cached
objects, leaving two orders of magnitude between the groups. At this size a
cache hit is actually *slower* for RABL than rebuilding from scratch, 53.2
against 70.7 — the effect reported against jbuilder in
[jbuilder#259](https://github.com/rails/jbuilder/issues/259), reproduced here.
The gap between the two byte-cachers also opens up at this size, 2.76x against
1.28x on the 22KB document. The RSS panel makes the deserialization cost
